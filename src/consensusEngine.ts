...
export interface ConsensusMetrics {
    ...
    agreementLevel: number; // 0.0 to 1.0
    confidenceVariance: number;
    convergenceTime: number; // milliseconds
    costEfficiency: number;
    consensusStrength: number; // weighted agreement score
}
...
export class ConsensusEngine {
    ...
    private simpleMajorityConsensus(votes: AgentVote[]): ConsensusResult {
        ...
        return {
            ...
            votes,
            metrics: {} as ConsensusMetrics, // Will be calculated later
            reasoning: `Simple majority consensus: ${maxVotes}/${votes.length} agents agreed on "${winner}"`,
            alternatives
        };
    }

    private weightedMajorityConsensus(votes: AgentVote[], options: ConsensusOptions): ConsensusResult {
    ...
    private confidenceWeightedConsensus(votes: AgentVote[]): ConsensusResult {
        const optionScores = new Map<string, number>();
        const optionConfidences = new Map<string, number[]>();
    ...
    private byzantineFaultTolerantConsensus(votes: AgentVote[]): ConsensusResult {
    ...
    private rankAggregationConsensus(votes: AgentVote[]): ConsensusResult {
    ...
    private fuzzyConsensus(votes: AgentVote[]): ConsensusResult {
    ...
    private costOptimizedConsensus(votes: AgentVote[]): ConsensusResult {
        // Optimize for cost-effectiveness while maintaining quality
        const costEfficiencyScores = new Map<string, number>();
    ...
    private calculateMetrics(votes: AgentVote[], result: ConsensusResult): ConsensusMetrics {
    ...
    private calculateAlternatives(votes: AgentVote[], winner: string): AlternativeOption[] {
        const options = new Map<string, AgentVote[]>();
    ...
    private removeOutliers(votes: AgentVote[]): AgentVote[] {
    ...
    private calculateByzantineConfidence(winnerGroup: AgentVote[], totalAgents: number, maxFaulty: number): number {
    ...
    private defaultWeightFunction(vote: AgentVote): number {
    ...
    async resolveConflicts(conflictingOutputs: ConflictingOutput[]): Promise<ConflictResolution> {
        this.outputChannel.appendLine(`Resolving ${conflictingOutputs.length} conflicting outputs`);
        
        // Convert conflicts to votes
        const votes: AgentVote[] = conflictingOutputs.map(output => ({
    ...
    private calculateConflictSeverity(votes: AgentVote[]): 'low' | 'medium' | 'high' {
        ...
        if (disagreementRatio < 0.7) return 'medium';
        return 'high';
    }
}

export interface ConsensusOptions {
    weightFunction?: (vote: AgentVote) => number;
    threshold?: number;
