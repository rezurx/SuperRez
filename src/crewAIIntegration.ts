...
import { AIOrchestrator, AITool } from './aiOrchestrator';
...
export class CrewAIIntegration {
    ...
    constructor(aiOrchestrator: AIOrchestrator, costTracker: CostTracker) {
        this.aiOrchestrator = aiOrchestrator;
        this.costTracker = costTracker;
    }

    async initializeCrewAI(): Promise<boolean> {
        try {
        ...
        return [
            {
                ...
                llm: 'auto', // Will be resolved to best available AI tool
                maxIterations: 3,
    ...
    async createMultiAgentTeam(
        ...
        const costApproval = await vscode.window.showWarningMessage(
            `Multi-AI Team execution will cost approximately $${costEstimate.toFixed(2)}. Continue?`,
            'Proceed', 'Cancel'
        );
        ...
        await this.costTracker.recordCost(
            'crewai',
            result.usageMetrics.totalTokens,
            result.usageMetrics.totalCost,
            `Multi-AI Team: ${taskDescription.substring(0, 50)}...`
        );
    ...
    private async resolveAgentLLMs(agents: Agent[]): Promise<Agent[]> {
    ...
    private isBackendRelated(description: string): boolean {
    ...
    private async estimateTeamCost(tasks: Task[], agents: Agent[]): Promise<number> {
        let totalCost = 0;
    ...
    private generateCrewAIPythonScript(
        agents: Agent[],
        tasks: Task[],
...
`;
    ...
    private async executeCrewAIScript(pythonScript: string): Promise<CrewResult> {
        try {
    ...
    async runConsensusVoting(
        ...
        options: string[],
        agents: string[]
    ): Promise<{ winner: string; votes: { [agent: string]: string }; confidence: number }> {
        const votes: { [agent: string]: string } = {};
    ...
    private async executeVote(prompt: string, toolName: string): Promise<string> {
