...
export interface LocalAIProvider {
    ...
    available: boolean;
    models: string[];
    costPerToken: number; // 0 for local models
}
...
export class LocalAIManager {
    ...
    async detectAvailableProviders(): Promise<LocalAIProvider[]> {
        const available: LocalAIProvider[] = [];
    ...
    async generateCode(provider: string, prompt: string, model?: string): Promise<string> {
    ...
    private async generateWithOllama(prompt: string, model: string): Promise<string> {
        const command = `ollama run ${model} "${prompt.replace(/"/g, '\\"')}"`;
        const { stdout } = await execAsync(command, { timeout: 30000 });
        return stdout.trim();
    }

    private async generateWithTransformers(prompt: string, model: string): Promise<string> {
...
`;
    ...
    private async generateWithMock(prompt: string): Promise<string> {
...
const codeReview = {
    ...
    async getProviderSummary(): Promise<string> {
        ...
        for (const provider of providers) {
            ...
            summary += `- **Status**: ${provider.available ? '✅ Available' : '❌ Not Available'}\n`;
        ...
        summary += `- 🆓 Zero API costs\n`;
        summary += `- 🔒 Complete privacy\n`;
        summary += `- ⚡ No internet required\n`;
